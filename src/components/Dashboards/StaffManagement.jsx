...
const StaffManagement = ({ user }) => {
  ...
  const fetchStaff = async () => {
    try {
      const res = await axios.get(`http://localhost:5000/api/staff/school/${user.schoolId}`);
  ...
  const handleAdd = async (e) => {
    e.preventDefault();
    try {
      await axios.post("http://localhost:5000/api/staff/add", { ...formData, schoolId: user.schoolId });
